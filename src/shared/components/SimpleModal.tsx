...
const SimpleModal = ({
  ...
  const transitionSeconds = animationDuration / 1000;
        ...
        <AnimatePresence>
          {isOpen && (
            <motion.div
              ...
              animate={{ opacity: 1 }}
              exit={{ opacity: 0 }}
              transition={{ duration: transitionSeconds, ease: "easeOut" }}
            >
              {/* Background */}
              <motion.div
              ...
              {/* Modal Content */}
              <motion.div
                className={`relative w-full ${width} bg-transparent max-h-[85vh] overflow-hidden
